...
menu=f"""\033[1;97m= = = = = = = = = = = = = = = = = = = = = = = = = = = = 
\033[1;37m┌─────────────────────┐
\033[1;32m║  \033[1;39m    INPUT TDS      \033[1;32m║
\033[1;37m└─────────────────────┘
\033[1;97m= = = = = = = = = = = = = = = = = = = = = = = = = = = = """
# Đánh dấu bản quyền
...
#while True:
	...
	while True:
		...
		logo()
		print(menu)
		dl = int(input(f'{thanh_xau}{trang}NHẬP DELAY: {luc}'))
		while True:
			if ntool == 2:
				break
			ntool = 0
			bongoc(14)
			nv_nhan=int(input(f'{thanh_xau}{trang}BAO NHIÊU JOB THÌ NHẬN XU: {luc}'))
			if nv_nhan < 8:
				print(f'{thanh_xau}{luc}KHÔNG ĐƯỢC NHỎ HƠN 8 !')
				continue
			if nv_nhan > 15:
				print(f'{thanh_xau}{luc}NHẬN XU DƯỚI 15 JOB KHÔNG LỖI !')
				continue
			bongoc(14)
			user_cau_hinh=input(f'{thanh_xau}{trang}USERNAME TIKTOK CẦN CHẠY: {luc}')
			cau_hinh=tds.run(user_cau_hinh)
			if cau_hinh != False:
				user=cau_hinh['uniqueID']
				id_acc=cau_hinh['id']
				bongoc(14)
				print(f'{trang}ID TIKTOK: {luc}{id_acc} {trang}| {trang}USER: {luc}{user} {trang}| ')
				bongoc(14)
			else:
				print(f'{thanh_xau}{luc}CẤU HINH THẤT BẠI USER: {trang}{user_cau_hinh} ')
				continue 
			while True:
